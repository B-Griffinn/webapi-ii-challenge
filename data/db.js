...
// returns a promise that resolves to an array of all the posts contained in the database.
function find() {
  return db('posts');
}

// expects an id as it's only parameter and returns the post corresponding to the id provided or an empty array if no post with that id is found.
function findById(id) {
  return db('posts').where({ id: Number(id) });
}

//  passing it a post object will add it to the database and return an object with the id of the inserted post. The object looks like this: { id: 123 }.
...
// accepts an id and returns the comment associated with that id.
...
// Calling insertComment while passing it a comment object will add it to the database and return an object with the id of the inserted comment.
